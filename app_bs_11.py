...
# --- 3. DATA (All original data restored) ---
...
if not all_images:
    ...
    if st.session_state.user_interest is None:
        ...
        # --- PRODUCT DETAIL VIEW: [Margin | Content | Gap | Content | Margin] ---
        item = st.session_state.user_interest
        if st.button("⬅ Back to Collection"):
            st.session_state.user_interest = None
            st.rerun()
            
        # 5 Columns: 0.6 Margin, 1.0 Image, 0.3 Middle Gap, 1.0 Buy Section, 0.6 Margin
        ...
        with c_buy:
            ...
            # Small, tight side-by-side buttons
            b1, b2 = st.columns(2)
