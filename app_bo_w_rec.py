...
if not all_images:
    ...
    if st.session_state.user_interest is None:
        # GRID VIEW
        for r in range(4): 
            # --- THE FIX: ADD 'and st.session_state.user_interest is None' ---
            if r == 1 and st.session_state.viewed_history and st.session_state.user_interest is None:
                with st.container():
                    ...
                    for i in range(5):
                        with rel_cols[i]:
                            if i < len(back_images):
                                ...
                                st.markdown(f'<p class="rating-text">{get_star_string(BACK_RATINGS[i])}</p>', unsafe_allow_html=True)
        ...
        with c_img:
            ...
            if len(all_images) > 16:
                sub1.image(all_images[16])
                sub2.image(all_images[17])
        ...
        with docker_slot.container():
            st.write("---")
            _, mid_dock, _ = st.columns([0.5, 3, 0.5])
            with mid_dock:
